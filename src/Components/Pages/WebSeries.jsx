...
function WebSeries() {
  ...
  let webSeriesData = homeCards.filter((val) =>
    val.homeCat.toLowerCase().includes("web-series")
  );
  let webSeriesPageLength = webSeriesData.length;
  let totalPage = Math.ceil(webSeriesPageLength / limit);
  ...
  let filterWebSeriesData = webSeriesData.slice(firstIndex, endIndex);
  return (
    <>
      <Header />
      <Sidebar />
      <div className="webSeriesContainer" style={containerStyle}>
        {filterWebSeriesData.map((val, i) => {
          return (
